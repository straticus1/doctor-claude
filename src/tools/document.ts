...
export const ReadMedicalDocumentSchema = z.object({
  path: z
    ...
    .min(1)
    .describe(
      'Absolute (or working-directory-relative) path to a LOCAL medical document: ' +
  ...
  documentType: z
    ...
    .default('auto')
    .describe(
      'Optional hint about the document kind. "auto" (default) infers handling from the ' +
 ...
 */
export async function readMedicalDocument(
  args: ReadMedicalDocumentInput
  ...
  if (fileStat.size > MAX_DOCUMENT_BYTES) {
    ...
    throw new Error(
      `File too large (${mb} MB). Maximum supported size is ${MAX_DOCUMENT_BYTES / (1024 * 1024)} MB.`
    );
  }
  ...
  if (ext === '.pdf') {
    ...
    if (!merged.trim()) {
      throw new Error(
        `No extractable text found in PDF: ${basename(absPath)}. ` +
  ...
  throw new Error(
    `Unsupported file type "${ext || '(none)'}" for ${basename(absPath)}. Supported: ${supported}.`
  );
}
