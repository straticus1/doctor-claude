...
import {
  readMedicalDocument,
  ReadMedicalDocumentSchema,
  type MedicalDocumentResult
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        },
        {
          ...
          description:
            'Read a LOCAL medical document so its contents can be discussed. ' +
          ...
          inputSchema: {
            ...
            properties: {
              path: {
                type: 'string',
                description:
                  'Path to the local file (PDF, image such as PNG/JPG, or text/CSV). Absolute paths recommended.',
              },
              documentType: {
                type: 'string',
                ...
                default: 'auto',
                description:
                  'Optional hint about the document kind. "auto" infers handling from the file extension.',
              },
            },
            required: ['path'],
          },
...
*This tool provides EDUCATIONAL information only. All clinical decisions must be made by qualified healthcare providers.*`;
          ...
          const doc: MedicalDocumentResult = await readMedicalDocument(validatedArgs);
  ...
  private formatDocument(doc: MedicalDocumentResult): Array<Record<string, unknown>> {
    const disclaimer =
      '*EDUCATIONAL use only. Extracted content is not a diagnostic-grade report; ' +
      'any imaging interpretation is preliminary and must be confirmed by a qualified clinician.*';

    if (doc.kind === 'image') {
      const header =
        `# Medical image: ${doc.path}\n` +
